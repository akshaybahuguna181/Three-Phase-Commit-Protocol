...
public class Participant3P {
	...
	/*flag to maintain state of participant*/
	private String state;
	
	/*Various flags to represent different states of Participant*/
		...
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					...
					Participant3P window = new Participant3P();
	...
	private void initialize() {
		...
		btnAbort.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if(!(stringdata.equals("")||stringdata.trim().isEmpty()||state.equals(precommit)||state.equals(ready))) {
		...
		JButton btnCommit = new JButton("Prepare Commit");
		btnCommit.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				
					/*check to prevent participant from voting more than once in middle of transaction*/
					if((state.equals(ready)||state.equals(precommit)||(stringdata.trim().isEmpty()||stringdata.equals("")))) {
						JOptionPane.showMessageDialog(null, "Can't Vote Now !");
					}
					else {
						...
						try {
							...
							/*state transition to Ready state*/
							state = ready;
							decision = "";
							...
							TimerTask tt2 = new TimerTask() {
								...
								@Override
								public void run() {
									/*check to see whether precommit or global abort decision has arrived or not*/
									if(!(decision.equals("p_c")||decision.equals("g_a"))) {
										chatArea.append("Did not receive decision from coordinator.\nSO LOCAL ABORT !\n");
										stringdata = "";
										state = "ABORT";
									}
								}
							};
							/*scheduling this check task every 40 seconds*/
							t.schedule(tt2, 40000);
		...
		btnCommit.setBounds(215, 231, 146, 37);
...
*/		JLabel lblEnterParticipantName = new JLabel("Enter Participant Name");
								...
								send connection request to server*/
									...
									if(clientname.equals(null)||clientname.trim().isEmpty()||(!Pattern.matches(regex, clientname)))
									{
										...
										TimerTask tt = new TimerTask() {
											...
											@Override
											public void run() {
												if(stringdata.equals("")&&state.equals("INIT")) {
													chatArea.append("Did Not get Voting Request.\nLocal Abort!\n");
													state = "ABORT";
												}
											}
										};
										/*scheduling this check task in (35 seconds or 35000 miliseconds)*/
										t.schedule(tt, 35000);
									}
								}
		...
		/*button to acknowledge the precommit message from coordinator*/
		JButton btnAck = new JButton("ACK");
		btnAck.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				if(state.equals(precommit)) {
					...
					/*Building the Http Connection Request and passing Client name as body. Thus the Http Header
				are encoded around the client name data.*/
					sbconnreq.append("POST /").append("_ACK_").append("/ HTTP/1.1\r\n").append(host).append("\r\n").
					append(userAgent).append("\r\n").append(contentType).append("\r\n").append(contentlength).append(clientname.length()).append("\r\n").
					append(date).append(new Date()).append("\r\n");
					...
					chatArea.append("Acknowledged the Commit !\n");
					
					/*3rd Timer for participant after it acknowledges the acknowledge message from coordinator.
					It waits for coordinator to send final Global Commit Decision within certain time, if not, it Commits the data.*/
					t = new Timer();
					...
					TimerTask tt3 = new TimerTask() {
						...
						@Override
						public void run() {
							/*check to see whether global commit decision has arrived or not*/
							if(!(decision.equals("g_c"))) {
								chatArea.append("Did not receive decision from coordinator after Pre-Commit.\nSO LOCAL COMMIT !\n");
								state = commit;
								try {
									bw.write(stringdata);
									bw.flush();
		...
		btnAck.setBounds(390, 231, 68, 37);
		frame.getContentPane().add(btnAck);
	}
	
	/*Requests the server for Connection by creating as stream socket for the server port number- 9998*/
			...
			while(true) {
				...
				readChat = dis.readUTF();
					...
					if(arr[0].contains("POST")) {
						...
						/*In case decision is pre commit*/
						else if(msgin.equalsIgnoreCase("PRE_COMMIT")) {
							
							t.cancel();
							t.purge();
							
							chatArea.append("PREPARE COMMIT phase. ACKNOWLEDGE the commit\n");
						//	JOptionPane.showMessageDialog(null, "PREPARE COMMIT phase. ACKNOWLEDGE the commit");
							
							/*State Transition from Ready to precommit after decision of coordinator*/
							state = precommit;
							decision = "p_c";
						...
						/*In case decision is global abort*/
							...
							stringdata = msgin.split(":")[0];
							chatArea.append("VOTE to COMMIT OR ABORT the string: "+stringdata+"\n");
